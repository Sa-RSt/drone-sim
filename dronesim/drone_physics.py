...
class DronePhysics:
    ...
    def total_torque(self, densities) -> np.ndarray:
        ...
        f1 = self.rot.apply(Rotation.from_euler('xyz', (0, 0, 0)).apply(np.array([0., 0., forces[1]])))
        f2 = self.rot.apply(Rotation.from_euler('xyz', (0, 0, 0)).apply(np.array([0., 0., forces[2]])))
        f3 = self.rot.apply(Rotation.from_euler('xyz', (0, 0, 0)).apply(np.array([0., 0., forces[3]])))
        ...
        omega = np.linalg.norm(rv)
        #breakpoint()
        """ if abs(omega) > 1e-9:
            air_res = 2 * self.resistive_drag * omega * omega * self.radius * self.radius
    ...
    def tick(self, dt):
        densities = 1. + self.air_density_randomness * 2 * (np.random.random(4) - .5)
    ...
    def tick_angular(self, dt, densities):
        ...
        tau = dt * self.total_torque(densities) / self.moment_of_inertia
